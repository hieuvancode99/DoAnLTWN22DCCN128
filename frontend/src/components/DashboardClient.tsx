...
export default function DashboardClient({ transactions, budgetsStatus }: DashboardClientProps) {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-white">Tổng quan tài chính</h1>
        <p className="text-slate-400 mt-1">Xin chào! Dưới đây là thống kê tình hình thu chi mới nhất của bạn.</p>
      </div>

      {/* Stats Cards */}
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
        {/* Balance Card */}
        <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/50 p-6 shadow-xl backdrop-blur-md">
          <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500 rounded-full mix-blend-multiply filter blur-3xl opacity-5"></div>
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-slate-400">Số dư hiện tại</span>
          ...
          <div className="mt-4">
            <span className="text-3xl font-bold tracking-tight text-white">
              {balance.toLocaleString('vi-VN')} <span className="text-lg font-normal text-slate-400">đ</span>
            </span>
          </div>
        ...
        {/* Income Card */}
        <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/50 p-6 shadow-xl backdrop-blur-md">
          <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500 rounded-full mix-blend-multiply filter blur-3xl opacity-5"></div>
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-slate-400">Tổng thu nhập</span>
          ...
          <div className="mt-4">
            <span className="text-3xl font-bold tracking-tight text-emerald-400">
              +{totalIncome.toLocaleString('vi-VN')} <span className="text-lg font-normal text-slate-400">đ</span>
            </span>
          </div>
          <div className="mt-2 text-xs text-slate-400">
        ...
        {/* Expense Card */}
        <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/50 p-6 shadow-xl backdrop-blur-md">
          <div className="absolute top-0 right-0 w-32 h-32 bg-rose-500 rounded-full mix-blend-multiply filter blur-3xl opacity-5"></div>
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-slate-400">Tổng chi tiêu</span>
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-600/10 text-rose-400">
              <TrendingDown className="h-5 w-5" />
            </div>
          </div>
          <div className="mt-4">
            <span className="text-3xl font-bold tracking-tight text-rose-400">
              -{totalExpense.toLocaleString('vi-VN')} <span className="text-lg font-normal text-slate-400">đ</span>
            </span>
          </div>
          <div className="mt-2 text-xs text-slate-400">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3 rounded-2xl border border-slate-800 bg-slate-900/50 p-6 shadow-xl backdrop-blur-md">
          <h2 className="text-lg font-semibold text-white mb-4">Biểu đồ thu chi tuần này</h2>
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-900/50 p-6 shadow-xl backdrop-blur-md">
          <h2 className="text-lg font-semibold text-white mb-4">Cơ cấu chi tiêu</h2>
          <div className="h-80 w-full flex flex-col justify-center items-center">
            {pieData.length > 0 ? (
              ...
              <div className="text-slate-400 text-sm">Chưa có chi tiêu nào được ghi nhận</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-900/50 p-6 shadow-xl backdrop-blur-md">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/transactions" className="flex items-center text-xs font-semibold text-indigo-400 hover:text-indigo-300">
          ...
          <div className="divide-y divide-slate-800/60">
            {recentTransactions.length > 0 ? (
                ...
                return (
                  <div key={tx._id} className="flex items-center justify-between py-3.5 first:pt-0 last:pb-0">
                    <div className="flex items-center gap-3">
                      <div 
                        className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-800"
                        style={{ backgroundColor: `${tx.categoryId?.color || '#334155'}15` }}
                      >
                        <Icon className="h-5 w-5" style={{ color: tx.categoryId?.color || '#94A3B8' }} />
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-white">{tx.description || tx.categoryId?.name}</p>
              ...
              <div className="py-8 text-center text-slate-400 text-sm">Chưa có giao dịch nào được lưu.</div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6 shadow-xl backdrop-blur-md">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white">Tiến độ ngân sách</h2>
            <Link href="/budgets" className="flex items-center text-xs font-semibold text-indigo-400 hover:text-indigo-300">
          ...
          <div className="space-y-4">
            {budgetsStatus.length > 0 ? (
                ...
                return (
                  <div key={budget._id} className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs font-medium">
                      <span className="text-slate-300">{budget.category?.name}</span>
              ...
              <div className="py-8 text-center text-slate-400 text-sm">Chưa thiết lập ngân sách tháng này.</div>
            )}
          </div>
        </div>
      </div>
